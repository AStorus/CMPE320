...
void testJumble() {
	...
	cout << "string being hidden is " << tstr << endl;
	puzzle = new JumblePuzzle(tstr, "easy");

	cout << "Testing size, should be " << tstr.length() * 2 << endl;
	cout << "Puzzle size: " << puzzle->getSize() << endl;
	cout << "The word should appear at row:col:dir \n" << puzzle->getRowPos() << "\n" << puzzle->getColPos() << "\n" << (int)puzzle->getDirection() << "\nwhere 0 = up, 1 = right, 2 = down, 3 = left" << endl;
		
	showJumble(puzzle->getJumble(), tstr.length() * 2);
	

	cout << "Testing exceptions:" << endl;
	try {
		JumblePuzzle shouldnotwork("wrong", "answer");
	}
	catch (BadJumbleException &e) {
		cerr << e.what() << endl;
		cout << "Passed error test 1" << endl;
	}
	try {
		JumblePuzzle shouldnotwork("thisstinrgistoolong", "easy");
	}
	catch (BadJumbleException &e) {
		cerr << e.what() << endl;
		cout << "Passed error test 2" << endl;
	}
